...
def exe():
    def arff_to_list(file_name, target_feature_name):
        ...
        # add fuzzy version and keep original version 
        ...
        for element in incomplete_list:
            for j in range(repeated_times):
                ...
                total_number = number_points * len(data[0][0])
                print("number instances",total_number)
                ...
                for i in mutation_index:
                    row = i //  len(data[0][0]) 
                    column = i - ( len(data[0][0]) * (row)) 
                    #print(row, column)
                    value = data[row][0][column]
                    #print("value is",value)
                    
                    data[row][0][column]  = 1- value     
                ...
                for step in range(n_fold):
                    ...
                    with open('../data/'+file_name+'/in_format/fuzzy/'+str(element)+'/'+str(j)+'/'+target_feature_name+str(step+1)+'Training.data', 'wb') as file_data:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
                        pickle.dump(training_data,file_data)
                        file_data.close()   
                    with open('../data/'+file_name+'/in_format/fuzzy/'+str(element)+'/'+str(j)+'/'+target_feature_name+str(step+1)+'Testing.data', 'wb') as file_data:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
                        pickle.dump(testing_data,file_data)
                        file_data.close()   
                    with open('../data/'+file_name+'/in_format/fuzzy/'+str(element)+'/'+str(j)+'/'+target_feature_name+str(step+1)+'Training.txt', 'w') as file_data:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
                        print(training_data,file = file_data)
                        file_data.close()   
                    with open('../data/'+file_name+'/in_format/fuzzy/'+str(element)+'/'+str(j)+'/'+target_feature_name+str(step+1)+'Testing.txt', 'w') as file_data:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
                    ...
                    with open('../data/'+file_name+'/in_format/fuzzy/'+str(element)+'/'+str(j)+'/'+file_name+'_'+str(step+1)+'.feature', 'wb') as file_feature:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
                        ...
                        file_data.close()
                create_arff(file_name, target_feature_name, data, attribute_name, element, j)
    ...
    def create_arff(file_name, task_name, data, attribute_name, element, repeate_times):
        ...
        wp = open('../data/'+file_name+"/jrip/fuzzy/"+str(element)+ '/' + str(repeate_times)+'/'+task_name+str(element)+".arff",'w')
        train = data
