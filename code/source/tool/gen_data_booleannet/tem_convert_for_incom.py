...
def exe(file_name):
    ...
    #file_name = ['tmax']
    def gen_data_incomplete(file_name):
        ...
        for element in incomplete_list:
            for j in range(repeated_times):
                with open('../data/'+file_name+'/in_format/train_with_negation.dt', 'rb') as file:
                    ...
                    #print("before shuffle", train_data)
                    random.shuffle(train_data)  
                    train_data = train_data[0: int(element * 0.01 * number_points) ]
                ...
                for step in range(n_fold):
                    ...
                    with open('../data/'+file_name+'/in_format/incomplete/'+str(element)+'/'+str(j)+'/'+file_name+str(step+1)+'Training.data', 'wb') as file_data:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
                        pickle.dump(training_data,file_data)
                        file_data.close()   
                    with open('../data/'+file_name+'/in_format/incomplete/'+str(element)+'/'+str(j)+'/'+file_name+str(step+1)+'Testing.data', 'wb') as file_data:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
                        pickle.dump(testing_data,file_data)
                        file_data.close()   
                    with open('../data/'+file_name+'/in_format/incomplete/'+str(element)+'/'+str(j)+'/'+file_name+str(step+1)+'Training.data.p2', 'wb') as file_data:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
                        pickle.dump(training_data,file_data, protocol=2)
                        file_data.close()   
                    with open('../data/'+file_name+'/in_format/incomplete/'+str(element)+'/'+str(j)+'/'+file_name+str(step+1)+'Testing.data.p2', 'wb') as file_data:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
                        pickle.dump(testing_data,file_data, protocol = 2)
                        file_data.close()   
                    with open('../data/'+file_name+'/in_format/incomplete/'+str(element)+'/'+str(j)+'/'+file_name+str(step+1)+'Training.txt', 'w') as file_data:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
                        print(training_data,file = file_data)
                        file_data.close()   
                    with open('../data/'+file_name+'/in_format/incomplete/'+str(element)+'/'+str(j)+'/'+file_name+str(step+1)+'Testing.txt', 'w') as file_data:  #save training data in format of [[[data],[label]],...,[[data],[label]]]
    ...
    def create_arff(file_name, task_name, data, attribute_name, element, repeate_times, head_variable_index, head_name, head_num):
        ...
        wp = open('../data/'+file_name+"/jrip/incomplete/"+str(element)+ '/' + str(repeate_times)+'/'+task_name+str(element)+head_name+".arff",'w')
        train = data
