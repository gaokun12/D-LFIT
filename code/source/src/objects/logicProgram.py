...
#--------------
    ...
    def return_var_list(self):
        features = list(self.__variables)
        return features
    ...
    def next(self, state):
        ...
        """
        output = [0 for i in self.__variables]   # default next value of each variable is 0

        for r in self.__rules:
            if output[r.get_head_variable()] == 1:  # Add by kun, Cause program is in DNF format
                continue                            # Add by kun, cause program is in DNF format
            if(r.matches(state)):
   #            output[r.get_head_variable()] = r.get_head_value()
                output[r.get_head_variable()] = r.get_head_value()
            else:
                output[r.get_head_variable()] = 0
